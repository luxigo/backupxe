...
if [ -n "$BACKUPXE" ] ; then
  for file in /lib/modules/`uname -r`/kernel/drivers/net/* /lib/modules/`uname -r`/kernel/drivers/net/pcmcia/*  ; do
    if [ -r $file ]; then
      OUTPUT=`basename $file .gz`
      OUTPUT=`basename $OUTPUT .o`
      CARDLIST= "$OUTPUT $CARDLIST"
    fi
  done
else
	...
	if [ "`echo $INPUT | crunch | cut -f 1 -d ' '`" = "L" \
	  ...
	  for file in /network/lib/modules/`uname -r`/kernel/drivers/net/* /network/lib/modules/`uname -r`/kernel/arch/i386/kernel/* /network/lib/modules/`uname -r`/kernel/drivers/pnp/* ; do
	    if [ -r $file ]; then
	      OUTPUT=`basename $file .gz`
	      OUTPUT=`basename $OUTPUT .o`
	      echo $OUTPUT
...
#  for card in \
#    3c59x acenic de4x5 dgrs eepro100 e1000 epic100 hp100 ne2k-pci olympic pcnet32 rcpci 8139too 8139cp sktr tulip via-rhine yellowfin tg3 dl2k ns83820 rtl8139 \
   if [ -z "$CARDLIST" ] ; then
	 CARDLIST="3c59x acenic de4x5 dgrs eepro100 e1000 epic100 hp100 ne2k-pci olympic pcnet32 rcpci 8139too 8139cp sktr tulip via-rhine yellowfin tg3 dl2k ns83820 rtl8139 sk98lin" ;
  fi
  for card in \
    $CARDLIST \
    ; do
...
echo "Preparing to disconnect the network supplemental disk.  Press"
echo "[enter] to unmount the disk."
echo
