...
if [ ! -d linux-$KVERSION ] || [ -f linux-$KVERSION.tar.bz2 ] ; then
	...
	rm -i linux-$KVERSION.tar.bz2 || true
...
if [ ! -f $WORKDIR/vmlinuz ] && [ ! -f $WORKDIR/bzImage ] ; then
	...
	[ -d $WORKDIR/modules/$KVERSION.bak ] && rm $WORKDIR/modules/$KVERSION.bak -r
	[ -d $WORKDIR/modules/$KVERSION ] && mv $WORKDIR/modules/$KVERSION $WORKDIR/modules/$KVERSION.bak
fi

if [ ! -d $WORKDIR/modules/$KVERSION ] ; then
...
if [ true ] ; then
	...
	if [ ! -d $WORKDIR/initrd_overlay/lib/modules/$KVERSION ] ; then
		if [ ! -z "`ls $WORKDIR/initrd_overlay/lib/modules/?.* || true `" ] ; then
			cp -a `ls -1d $WORKDIR/initrd_overlay/lib/modules/?.* | tail -n 1`  $WORKDIR/initrd_overlay/lib/modules/$KVERSION
		fi
	...
	if [ -d $WORKDIR/initrd_overlay/lib/modules/$KVERSION/ ] ; then
		...
		cp -a $KVERSION $WORKDIR/initrd_overlay/lib/modules
	fi
fi
